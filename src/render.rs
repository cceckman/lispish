...
use dot_writer::{DotWriter, Node, Scope};
...
/// Render the state of storage as a Graphviz graph.
pub fn render_store<'a>(store: &'a Storage, labeled_nodes: impl IntoIterator<Item = (Ptr<'a>, &'a str)>) -> Vec<u8> {
        ...
        let mut outbuf = Vec::new();
        {
            ...
            while let Some(it) = queue.pop_front() {
                ...
                match store.get(it) {
                    Object::Nil => continue,
                    Object::Integer(v) => { node.set_label(&format!("{{{name}|{v}}}")); },
                    Object::Float(v) => { node.set_label(&format!("{{{name}|{v}}}")); },
                    Object::String(v) => {
